...
from lib.srpe import SRPENet
from lib.loss import *
from dataset.pose_data import PoseDataset
from utils import setup_logger, compute_sRT_errors
...
def pre_train():
    ...
    # model & loss
    estimator = SRPENet(opt.n_cat, opt.nv_prior, num_structure_points=256)
    estimator.cuda()
    ...
    for epoch in range(opt.start_epoch, opt.max_epoch + 1):
        ...
        for i, data in enumerate(train_dataloader, 1):
            ...
            if i % 10 == 0:
                logger.info(
                    'Batch {0} Loss:{1:f}, corr_loss:{2:f}, cd_loss:{3:f}, entropy_loss:{4:f}, deform_loss:{5:f}, lowrank_loss:{6:f}'.format(
